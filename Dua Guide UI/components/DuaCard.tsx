...
import { getSpiritualInsight } from '../geminiService';
...
const DuaCard: React.FC<DuaCardProps> = ({ dua, categoryName }) => {
  const [insight, setInsight] = useState<string>('Generating spiritual insight...');
  const [loadingInsight, setLoadingInsight] = useState(true);
  const [bookmarked, setBookmarked] = useState(isBookmarked(dua.id));

  useEffect(() => {
    let isMounted = true;
    const fetchInsight = async () => {
      setLoadingInsight(true);
      const res = await getSpiritualInsight(dua.title, dua.arabic);
      if (isMounted) {
        setInsight(res);
        ...
        <div className="space-y-8">
          ...
          <div className="bg-amber-50/30 rounded-2xl p-5 border border-amber-100/50">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h4 className="text-[10px] font-bold text-amber-800/60 uppercase tracking-widest">Spiritual Insight</h4>
            </div>
            <p className={`text-slate-700 text-sm leading-relaxed ${loadingInsight ? 'animate-pulse' : ''}`}>
              {insight}
            </p>
          </div>
        </div>
